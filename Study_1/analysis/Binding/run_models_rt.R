...
# Estimate ----------------------------------------------------------------
  ...
  m0_fit <- brm(
    retweet_count ~ stance + (1|user_id), 
    family = "negbinomial", 
  ...
  m1_fit <- brm(
    retweet_count ~ binding + individual + stance + (1|user_id),    family = "negbinomial", 
    data = df_total_binding,
    chains = 8,
    iter = 1500,
    warmup = 750,
  ...
  m2_fit <- brm(
    retweet_count ~ (binding + individual)*stance + (1|user_id),
    family = "negbinomial", 
    data = df_total_binding,
    chains = 8,
    iter = 1500,
  ...
  # Save results as .rds
  write_rds(m0_fit, "results/m0_fit_rt.rds")
  write_rds(m1_fit, "results/m1_fit_rt.rds")
  write_rds(m2_fit, "results/m2_fit_rt.rds")
...
# Export ------------------------------------------------------------------
  ...
  # Save results as .rds
  write_rds(m0_fit, "../results/binding/m0_fit_rt.rds")
  write_rds(m1_fit, "../results/binding/m1_fit_rt.rds")
  write_rds(m2_fit, "../results/binding/m2_fit_rt.rds")
  
  # Main Results ------------------------------------------------------------
  ######## Extract coefficients
  m2_fit_results <- m2_fit %>% 
    spread_draws(
      `b_binding1`,
      `b_individual1`,
  ...
  write_rds(m2_fit_results, "../results/binding/m2_rt_effects.rds")
  
# Model comparison ---------------------------------------------------------

  # Load results
  m0 <- read_rds("../results/binding/m0_fit_rt.rds")
  m1 <- read_rds("../results/binding/m1_fit_rt.rds")
  m2 <- read_rds("../results/binding/m2_fit_rt.rds")
...
# Initialize models
  results <- crossing(
    ...
    mutate(
      fit0 = map(model0, ~read_rds(paste0("../results/binding/", str_to_lower(.), "_fit_rt.rds"))),
      fit1 = map(model1, ~read_rds(paste0("../results/binding/", str_to_lower(.), "_fit_rt.rds")))
    )
...
# Calculate ELPD differences
  ...
  # Save results as .rds
  write_rds(results_comparison, "../results/binding/comparison_rt.rds")
